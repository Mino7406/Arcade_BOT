...
- **틱택토(`/틱택토`)** — 1v1(레디제) 또는 봇 대결(무한모드 선택 가능) 틱택토, 미입력 시 5분 자동 종료, 사람끼리 붙으면 자동으로 XP 내기(레벨 강등 방지 상한 있음) (지정된 채널 전용)
...
- `index.js`의 `WORDCHAIN_RANKING_CHANNEL_ID`로 지정된 채널에서만 사용 가능
- `상대방` 옵션 없이 실행하면 **봇전 설정 로비**(무한모드 토글 + `▶️ 시작`)가 먼저 뜸(60초 내 미시작 시 취소), 옵션으로 유저를 지정하면 **레디 로비**(둘 다 `✅ 레디`를 눌러야 시작, `❌ 거절`로 즉시 취소 가능)가 뜸(60초 내 둘 다 레디 안 하면 자동 만료) — 로비 단계에서 참가자 누구든 `무한모드: ON/OFF`를 토글할 수 있고, PvP 로비에서 토글하면 규칙이 바뀌었으므로 양쪽 레디가 초기화됨
...
- 게임이 끝나면 보드 아래에 `🔄 재대결` / `🛑 종료` 버튼이 붙음(원래 참가자만 사용 가능) — 재대결은 봇전이면 무한모드를 다시 고르는 설정 로비로, 사람 상대였다면 선후공을 서로 바꿔(X↔O) 레디 로비로 다시 진입(무한모드 여부는 직전 판 설정을 그대로 이어받음)
...
| 함수 | 설명 |
|---|---|
| `startTttCommand(interaction)` | `/틱택토` 실행 → `상대방` 유무에 따라 레디 로비(PvP) 또는 봇전 설정 로비를 게시 |
| `handleTttButton(interaction)` | 레디/거절/무한모드 토글/봇 시작/칸 선택/재대결/종료(`ttt:ready:`, `ttt:decline:`, `ttt:toggleinf:`, `ttt:botstart:`, `ttt:move:`, `ttt:rematch:`, `ttt:close:`) 버튼 처리 |
| `startRematch(interaction, prevXId, prevOId, infinite)` *(내부)* | `🔄 재대결` 클릭 시 무한모드 여부를 이어받아 설정/레디 로비를 새로 생성(사람 상대는 선후공을 바꿔 진행) |
