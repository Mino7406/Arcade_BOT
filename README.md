...
| `XP_TTS_CHANNEL_IDS` | TTS 채널 목록(메인 채널과 쿨다운·1회 지급량이 따로 정해져 있음) — 통화방 XP 중복 지급 방지 대상 구분용으로도 쓰임 |
...
각 경로의 쿨다운과 1회 지급량은 서로 독립적으로 정해져 있어(`MESSAGE_XP`·`randomVoiceXp()`·`randomMatchBonusXp()`/`ORGANIZER_BONUS_XP`) 한쪽을 튜닝해도 다른 경로가 같이 흔들리지 않는다. 메시지 쿨다운은 메인/TTS 채널이 각자 따로 셈한다.

| 경로 | 쿨다운 | 1회 지급 | 시간당 기대치 | 비고 |
|---|---|---|---|---|
| 메인 텍스트 채널 | 10분 | 6~11 (평균 8.5) | 최대 6회 ≈ 51 XP | 지정된 채널에서만 인정 |
| TTS 채널(통화 연동 전용 채팅방) | 3분 | 3~7 (평균 5) | 최대 20회 = 100 XP | 이미 음성 XP를 받는 유저는 중복 지급 안 함 |
| 통화방(음성) 체류 | 1분마다 자동 지급 | 2~3 (평균 2.5) | 60틱 = 150 XP | 음소거/헤드셋오프 시 제외 |
| 내전/모집 완료 보너스 | 1회성 | 참가자 30~50 랜덤(평균 40) / 주최자 50 고정 | - | 지정된 채널의 매치가 마감될 때 지급, 유저당 중복 지급 없음 |
...
| `handleMessageXp(message)` | 메시지 1건에 대해 채널 종류(메인/TTS)별 쿨다운을 판정해 XP 지급 |
