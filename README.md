...
| `/랭킹` | 서버 XP 랭킹을 확인합니다 | - |
| `/xp` | [관리자 전용] 유저의 XP를 수동으로 더하거나 뺍니다 (본인에게만 보이는 결과, 어느 채널에서나 사용 가능) | `유저`(필수), `양`(필수, 음수 가능) |
...
- **`/xp`**(관리자 전용, `ADMIN_IDS`) — `유저`와 `양`(음수 가능)을 받아 `adjustXp`로 즉시 가감하고 `saveLevels()`로 바로 저장, 조정 전후 XP·레벨과 레벨업/하락 여부를 본인에게만(ephemeral) 보여줌. 최종 XP가 0 밑으로는 내려가지 않게 잘라내며(음수 XP는 레벨·진행바 계산을 깨뜨림), 잘린 경우 실제 적용값을 함께 안내. `/패널`·`/퀴즈`처럼 채널 제한을 받지 않아 아무 채널에서나 사용 가능
