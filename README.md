...
├─ 퀴즈.json                 # 오늘 출제 시각/모드(초성·상식)/출제 여부/최근 출제 단어 기록 (자동 생성, gitignore)
...
- **XP 내기/보상**: 참가자가 전부 사람이면 자동으로 내기가 걸림 — 탈락자가 min(`WAGER_XP`=100, 탈락자의 **현재 레벨 안에 쌓인 XP**)만큼 잃고, 그 XP를 생존자들이 똑같이 나눠 가짐(레벨 자체가 깎이는 일은 없음). 참가자 중 봇이 있었고 그 봇이 탈락했다면(=사람들이 이김) 내기 대신 생존자 각자에게 고정 `BOT_WIN_XP`=60을 지급(사람이 진 경우는 봇전이라 페널티 없음). 결과 임베드에 정산 내역이 함께 표시되고, 정산으로 레벨업하면 평소처럼 레벨업 채널에 축하 메시지가 감

### 틱택토 (`/틱택토`, `handlers/틱택토.js`)

- `index.js`의 `WORDCHAIN_RANKING_CHANNEL_ID`로 지정된 채널에서만 사용 가능
- `상대방` 옵션 없이 실행하면 봇과 즉시 대결 시작(항상 선공), 옵션으로 유저를 지정하면 대결 신청 메시지(✅ 수락 / ❌ 거절) 게시 후 60초 내 미응답 시 자동 만료
- 참가자만 자신의 차례에 버튼으로 칸을 선택 가능, 한 수도 없이 5분이 지나면 시간 초과로 무승부 종료
- 봇 로직: 이길 수 있는 자리 → 상대 승리를 막는 자리 → 중앙 → 코너 → 남은 칸 순으로 우선순위 결정
...
- 매일 KST 기준 12:00~23:00 사이 무작위 시각에 놀이터 채널(`QUIZ_CHANNEL_ID`)에 자동으로 1문제 출제, 하루 최대 1회
...
- **`/퀴즈`**(관리자 전용, `ADMIN_IDS`)를 실행하면 옵션 없이 바로 현재 상태(중지 여부/오늘 모드/출제 여부·예정 시각/미해결 문제)와 조작 버튼이 담긴 패널이 본인에게만 표시됨. 버튼으로 전부 조작:
  - `⏸️ 중지` / `▶️ 재개` — 예약된 자동 출제를 껐다 켰다 함(중지 중에도 이미 나가 있는 문제의 채점은 계속 동작). 재개하면 즉시 재스케줄 확인(날짜가 넘어가 있었다면 그 자리에서 오늘 몫을 다시 계산)
  - `✍️ 문제 만들기` → 초성퀴즈/상식퀴즈 중 모드를 고르면 **단어·힌트를 직접 입력하는 모달**이 뜸(사전 API로 무작위로 고르지 않고 관리자가 쓴 그대로 출제) — 초성퀴즈는 입력한 단어에서 초성을 자동 계산해 보여줌. 중지 상태이거나 오늘 이미 출제된 상태여도 무시하고 강제로 새로 냄(그 전 미해결 문제는 평소처럼 무효 처리 후 진행)
...
| `getBotMove(board)` *(내부)* | 봇의 다음 수 결정: 승리 → 방어 → 중앙 → 코너 → 랜덤 순 |
...
| `voidPreviousQuiz(client, state)` *(내부)* | 새 문제를 내기 직전, 전날 문제가 안 풀린 채 남아있으면 정답을 공개하고 보상 없이 무효 처리 |
| `getChosung(word)` *(내부)* | 단어를 초성만 남긴 문자열로 변환 |
| `checkAndSchedule(client)` *(내부)* | 오늘(KST) 출제 여부/예정 시각/모드를 `퀴즈.json` 기준으로 확인하고 필요시 새로 예약(모드는 전날의 반대로 자동 결정), `paused` 상태면 예약을 걸지 않음 |
| `pauseQuiz()` | `/퀴즈`의 `⏸️ 중지` 버튼 — 예약된 타이머를 취소하고 `paused: true`를 저장 |
| `resumeQuiz(client)` | `/퀴즈`의 `▶️ 재개` 버튼 — `paused: false`로 되돌리고 `checkAndSchedule`을 즉시 한 번 더 호출 |
| `postCustomQuiz(client, modeOverride, word, hint)` | `/퀴즈`의 `✍️ 문제 만들기` 모달 제출 — API로 무작위로 고르지 않고 관리자가 입력한 단어·힌트 그대로 즉시 출제(예약 타이머·오늘 출제 여부와 무관) |
| `prepareManualFire(modeOverride)` *(내부)* | 수동 출제(직접 만들기) 공통 준비 로직 — 예약 타이머 해제, 오늘 상태 로드/갱신, `posted: true` 선반영 |
| `getQuizStatus()` | `/퀴즈` 패널 — `퀴즈.json`에 저장된 현재 상태(모드/출제 여부/예정 시각/미해결 문제 등)를 그대로 반환 |
...
| `buildCreateMenuPayload()` | `✍️ 문제 만들기` 클릭 시 모드 선택 버튼 화면 |
