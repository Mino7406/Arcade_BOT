...
- **틱택토(`/틱택토`)** — 1v1(수락/거절) 또는 봇 대결 틱택토, 미입력 시 5분 자동 종료, 사람끼리 붙으면 자동으로 XP 내기(레벨 강등 방지 상한 있음) (지정된 채널 전용)
- **초성퀴즈·상식퀴즈** — 놀이터 채널에 하루 한 번 무작위 시각으로 자동 출제, 두 방식을 매일 번갈아가며 진행(초성퀴즈: 초성+뜻풀이 힌트 / 상식퀴즈: 뜻풀이만, 더 어려움). 국립국어원 API에서 매번 다른 단어를 가져와 고정/반복되지 않음, 가장 먼저 맞힌 사람에게 대량 XP 지급
...
├─ 퀴즈.json                 # 오늘 출제 시각/모드(초성·상식)/출제 여부/최근 출제 단어/자동·관리자 미해결 문제 기록 (자동 생성, gitignore)
...
- `index.js`의 `WORDCHAIN_RANKING_CHANNEL_ID`로 지정된 채널에서만 사용 가능
- `상대방` 옵션 없이 실행하면 봇과 즉시 대결 시작(항상 선공), 옵션으로 유저를 지정하면 대결 신청 메시지(✅ 수락 / ❌ 거절) 게시 후 60초 내 미응답 시 자동 만료(신청 임베드에 내기가 걸린다는 안내 표시)
- 참가자만 자신의 차례에 버튼으로 칸을 선택 가능, 한 수도 없이 5분이 지나면 시간 초과로 무승부 종료
- **봇 로직은 완전 탐색(미니맥스)** — 3x3 보드가 작아 매 수마다 모든 경우의 수를 다 봐서 이론상 절대 지지 않음(최악이 무승부). 예전엔 단순 휴리스틱이라 "상대 코너 → 내 중앙 → 상대 반대편 코너"로 시작하는 유명한 트릭에 뚫려 몇 수만 외우면 반복해서 이길 수 있었는데, 완전 탐색으로 바꿔서 봇전 승리 XP 파밍 자체를 원천 차단
- **XP 내기/보상**: 사람 vs 사람 대결에서 승부가 나면(무승부 제외) 자동으로 내기가 걸림 — 진 사람이 min(`WAGER_XP`=100, 진 사람의 **현재 레벨 안에 쌓인 XP**)만큼 잃고 이긴 사람이 그만큼 얻음(레벨 자체가 깎이는 일은 없음). 사람이 봇을 이기면 내기 대신 고정 `BOT_WIN_XP`=60을 지급, 봇이 이기거나 무승부면 아무 변동 없음. 결과 임베드에 정산 내역이 함께 표시되고, 정산으로 레벨업하면 평소처럼 레벨업 채널에 축하 메시지가 감
- **악용 방지**: 유저당 `XP_SETTLE_COOLDOWN_MS`(3분) 쿨다운으로 봇전 반복 파밍이나 같은 상대와의 즉석 내기 반복을 제한(쿨다운 중에도 게임 자체는 정상 진행되지만 XP 정산만 생략됨), 여기에 위 미니맥스 봇까지 더해져 봇전 승리 자체가 우연/실력에 좌우돼 반복 파밍이 사실상 불가능

### 초성퀴즈·상식퀴즈 (`handlers/퀴즈.js`, 관리자 전용 제어 커맨드는 `/퀴즈`)

- 매일 KST 기준 낮 12시~다음날 새벽 5시 사이 무작위 시각에 놀이터 채널(`QUIZ_CHANNEL_ID`)에 자동으로 1문제 출제, 하루 최대 1회(유저 활동이 새벽 시간대에 몰리는 경우가 많아 자정 이후까지 포함)
- **초성퀴즈**와 **상식퀴즈**를 매일 번갈아 진행(전날 출제된 모드의 반대로 자동 결정, 최초 실행 시 초성퀴즈부터) — 두 모드 모두 같은 난이도(초/중급)의 단어를 뽑고 보상 XP도 동일(600), 유일한 차이는 상식퀴즈에 초성 힌트가 없다는 것뿐(그만큼 체감 난이도만 더 높음)
...
- 출제 예정 시각·모드·풀리지 않은 문제는 `퀴즈.json`에 고정 기록되므로, 봇이 재시작돼도 같은 시각·같은 모드로 다시 예약되고(이미 지났으면 즉시 출제) 채점도 끊기지 않고 이어짐, 하루에 두 번 출제되지 않음
...
| 함수 | 설명 |
|---|---|
| `startTttCommand(interaction)` | `/틱택토` 실행 → `상대방` 유무에 따라 대결 신청 또는 봇 대결 즉시 시작 |
| `handleTttButton(interaction)` | 수락/거절/칸 선택(`ttt:accept:`, `ttt:decline:`, `ttt:move:`) 버튼 처리 |
| `getBotMove(board)` / `minimax(board, isBotTurn)` *(내부)* | 미니맥스 완전 탐색으로 봇의 최선의 수를 결정(이론상 절대 지지 않음) |
...
| `checkAndSchedule(client)` *(내부)* | 오늘(KST) 출제 여부/예정 시각/모드를 `퀴즈.json` 기준으로 확인하고 필요시 새로 예약(모드는 전날의 반대로 자동 결정), `paused` 상태면 예약을 걸지 않음. 재시작 시 관리자 출제(`activeManualQuiz`)의 1시간 제한시간도 `expiresAt` 기준으로 남은 만큼 다시 예약(이미 지났으면 즉시 마감) |
...
| `getQuizStatus()` | `/퀴즈` 패널 — `퀴즈.json`에 저장된 현재 상태(모드/출제 여부/예정 시각/자동·관리자 미해결 문제 등)를 그대로 반환 |
...
- `/끝말잇기`, `/틱택토`, `/레벨`, `/랭킹`과 관련 버튼은 `ALLOWED_CHANNEL_ID`와 무관하게 `index.js`의 `WORDCHAIN_RANKING_CHANNEL_ID` 채널에서만 사용할 수 있습니다.
