...
- **한방단어(다음 사람이 이을 단어가 없는 단어) 차단** — 단어를 입력하면 그 마지막 글자로(두음법칙 반영) 시작하는 다른 단어가 사전에 하나라도 남아 있는지(이미 쓴 단어는 제외) 한국어기초사전 API로 함께 조회한다. 하나도 없으면 상대를 그 자리에서 막아버리는 한방단어로 보고 거부한다(사전 실존 여부 검증과 병렬로 조회해 추가 지연 없음). 조회에 전부 실패하면(예: `KRDICT_API_KEY` 없음) 부당하게 막지 않도록 통과시킨다(fail-open)
