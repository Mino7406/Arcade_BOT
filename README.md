...
| `XP_CHANNEL_MULTIPLIERS` | TTS 채널 XP 배율(0.06배, 메인 채널 기본 배율과 동일) — TTS 전용 쿨다운·통화방 XP 중복 지급 방지 대상 구분용으로도 쓰임 |
| `MATCH_BONUS_CHANNEL_ID` | 내전/모집 완료 보너스 XP 채널(= 인증 채널). `/불러오기` 전용 채널이기도 함 |
| `NEWBIE_BOOST_ROLE_ID` | `뉴비부스트` 역할. 보유자는 메인/TTS 메시지 XP·통화방 체류 XP를 1.5배로 받음(미니게임·완료 보너스 제외) |
...
| 경로 | 배율 | 쿨다운 | 비고 |
|---|---|---|---|
| 메인 텍스트 채널 | 0.06배 (15~25 XP) | 60초 | 지정된 채널에서만 인정 |
| TTS 채널(통화 연동 전용 채팅방) | 0.06배 | 3분 | 메인 채널과 동일 배율. 통화방 체류 XP와 시간당 수익을 동일하게 맞춤(시간당 평균 약 24XP), 이미 음성 XP를 받는 유저는 중복 지급 안 함 |
...
- `뉴비부스트` 역할(`NEWBIE_BOOST_ROLE_ID`) 보유자는 **메인 텍스트 채널·TTS 채널 메시지 XP와 통화방 체류 XP를 1.5배**로 받음(미니게임·내전/모집 완료 보너스에는 적용 안 됨)
