...
├─ migrate-db.js            # 1회성: 예전 루트에 흩어져 있던 JSON 저장 파일을 DB/ 폴더로 이동(내용 변경 없음)
...
> **기존에 봇을 돌리고 있었다면**: `data.json`·`levels.json` 등 JSON 저장 파일이 프로젝트 루트에 이미 쌓여 있을 수 있습니다. 새 코드는 이 파일들을 `DB/` 폴더(일부는 새 이름, 아래 [폴더 구조](#폴더-구조) 참고)에서 찾으므로, 코드를 갱신한 뒤 봇을 다시 켜기 전에 한 번만 `node migrate-db.js`를 실행해 옮겨주세요. 내용을 다시 쓰지 않고 파일시스템 레벨로 옮기기만 해서 데이터가 손상되지 않으며, 이미 옮겨진 파일은 건너뛰므로 여러 번 실행해도 안전합니다. 새로 설치하는 경우엔 필요 없습니다(봇이 처음 켜질 때 `DB/` 폴더를 알아서 만듭니다).
