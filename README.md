...
- "⚙️ 관리" 버튼(관리자 전용, `ADMIN_IDS`)을 누르면 관리자에게만(ephemeral) 패널 관리 메뉴가 표시됨. 이 메뉴 안의 모든 버튼/셀렉트는 공개 패널 버튼(`recruit:`)과 구분되는 `panel:` 프리픽스를 쓰며, 어느 패널 메시지를 대상으로 하는지 커스텀ID에 실린 `panelMessageId`로 특정함:
  ...
  - **🗑️ 내전/모집 삭제** — 현재 서버의 진행 중/마감된 내전·모집은 물론, 취소되어 자동삭제 대기 중인 게시글(🔴 취소됨, `client.cancelledDeletions`)까지 셀렉트 메뉴로 함께 표시 → 선택 후 확인 화면을 거쳐 삭제. 활성 매치는 알림 타이머 취소·완료 보너스 XP 지급·참가자 멘션 메시지 삭제까지 정리 후 삭제(관리 목록에서도 제거), 취소된 게시글은 자동삭제 대기열에서 제거 후 즉시 삭제. 목록 화면 하단의 **🗑️ 전체 삭제** 버튼으로 확인 화면을 한 번 거쳐 현재 서버의 표시된 항목을 한 번에 모두 삭제할 수도 있음
...
- **악용 방지**: 봇전 반복 파밍이나 같은 상대와의 즉석 내기 반복(예: "🏳️ 포기"로 즉시 종료해 XP만 옮기기)을 막기 위해 유저당 `XP_SETTLE_COOLDOWN_MS`(3분) 쿨다운을 둠 — 쿨다운 중에도 게임 자체는 정상 진행되지만 XP 정산만 생략됨
...
- **초성퀴즈**와 **상식퀴즈**를 매일 번갈아 진행(전날 출제된 모드의 반대로 자동 결정, 최초 실행 시 초성퀴즈부터) — 두 모드 모두 같은 난이도(초/중/고급, 2~6글자)의 단어를 뽑고, 보상 XP는 문제마다 100~300 사이에서 무작위로 정해져 정답자에게 실제 지급액이 그대로 안내됨. 유일한 차이는 상식퀴즈에 초성 힌트가 없다는 것뿐(그만큼 체감 난이도만 더 높음)
...
| `endGame(game, games, loserId, reason, failWord)` *(내부)* | 게임 종료 처리(`settleGameXp` 호출 후) 및 결과 임베드로 전환, 재대결 만료 타이머 설정 |
