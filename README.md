...
│  ├─ 레벨.js                # /레벨
│  ├─ 랭킹.js                # /랭킹
│  ├─ XP.js                  # /xp [관리자 전용] XP 조정(유저 선택→버튼, 유저별 지급 정지 포함) / XP 관리(긴급정지·뉴비부스트 토글)
│  └─ 신청서.js              # /신청서 [관리자 전용] 게시할 신청서를 버튼으로 골라 패널 게시 (진입점, 폼별 로직은 forms/)
├─ forms/                   # /신청서로 게시하는 신청서(폼)별 로직 — 폼마다 폴더 하나
│  └─ 마크/                  # 마인크래프트 렐름 참가 신청 폼
│     ├─ index.js            # 패널·신청 모달·검토 임베드·명단 관리 상호작용 ('realm:' / 'realm:roster:')
│     └─ 명단.js             # 승인 명단·검토 대기 신청·명단 메시지 ID 저장·조회 (realm_roster.json)
...
│  ├─ 끝말잇기.js
│  ├─ 틱택토.js
│  ├─ 오목.js                # /오목 — 게임 로직 + 판 PNG 렌더링 + 봇 AI 한 파일에 통합
│  ├─ 룰렛.js
...
| `XP_TTS_CHANNEL_IDS` | TTS 채널 목록(메인 채널과 배율·쿨다운 완전히 동일, 배율 자체가 없음) — 통화방 XP 중복 지급 방지 대상 구분용으로도 쓰임 |
| `MATCH_BONUS_CHANNEL_ID` | 내전/모집 완료 보너스 XP 채널(= 인증 채널). `/불러오기` 전용 채널이기도 함 |
| `NEWBIE_BOOST_ROLE_ID` | `뉴비부스트` 역할. 보유자는 메인/TTS 메시지 XP·통화방 체류 XP를 2배로 받음(미니게임·완료 보너스 제외) |
...
| `/오목` | 오목 대기 로비를 엽니다 (참가 버튼으로 합류·최대 2명, 방장이 시작 / 혼자면 봇과 시작). 착수는 자기 차례에 좌표를 채팅 입력(예: `H8`) | - |
...
| `/랭킹` | 서버 XP 랭킹을 확인합니다 | - |
| `/xp` | [관리자 전용] `XP 조정`(유저 선택 → 버튼으로 XP/레벨 수동 조정, 유저별 XP 지급 정지 토글) · `XP 관리`(일반파밍·미니게임 긴급정지, 뉴비부스트 ON/OFF)로 나뉜 패널 (본인에게만 보임, 어느 채널에서나) | 없음 (실행 → 버튼) |
...
- **착수는 채팅 좌표 입력** — 15×15는 버튼 격자(디스코드 한 메시지당 버튼 25개 한계)로 만들 수 없어서, 자기 차례에 `H8`·`8H`·`h-8` 같은 좌표를 채팅으로 보내면 그 자리에 돌이 놓임(`parseCoord`). 착수가 반영되면 그 메시지에 ✅를 달았다가 **메시지를 삭제**해 채널을 깔끔하게 유지한다(삭제엔 봇의 '메시지 관리' 권한 필요 — 없으면 그대로 남음). 좌표로 안 보이는 메시지는 잡담으로 간주해 무시(제한 시간은 계속 흐름), 이미 돌이 있는 자리면 4초 뒤 지워지는 경고를 답장으로 붙임. 끝말잇기의 채팅 핸들러(`handleWcMessage`)와 같은 방식
- **진행 중 `🏳️ 포기` 버튼** — 대국 참가자면 자기 차례가 아니어도 누를 수 있고, 누른 사람이 패(상대 승). 사람끼리면 포기도 내기 XP가 정산되고(끝말잇기 포기와 동일), 봇전 포기는 봇 승이라 XP 변동 없음
- **보드를 항상 맨 아래로** — 채팅(잡담·재대결 안내 등)이 쌓이면 보드 임베드가 위로 밀린다. `messagesSinceBoard`로 밀린 정도를 세고, 밀렸으면 기존 보드를 지우고 맨 아래에 다시 올린다(`moveBoardDown`, 끝말잇기 `refreshBoard`와 동일). 착수 메시지는 삭제되므로 카운트에서 되돌려(`-1`) 잡담이 없으면 제자리에서 갱신만 하고 불필요한 재게시를 피한다. 종료 화면은 내리기에 실패해도 원래 자리에서 갱신되도록 재시도(`editWithRetry`)
...
- **봇 AI는 패턴 점수 기반**(`pickBotMove`) — "오목 경우의 수 10^180"을 다 저장·탐색하는 게 아니라, 후보 칸(기존 돌 주변 2칸 이내)마다 4방향 연속선의 (길이 + 양끝 열림)을 점수로 환산(`cellScore`)해서 **내 공격 점수 + 상대 공격 점수 × 0.95**가 가장 큰 칸을 고름. 즉시 5는 무조건 두고/막고, 상대 열린 4도 무조건 막음. 한 수 계산 ~0.3ms
...
메시지·통화방 체류·내전/모집 완료 보너스는 서로 다른 전용 기본치를 쓴다(`randomMessageXp()` 15~25 / `randomVoiceXp()` 15~25 / `randomMatchBonusXp()` 30~50) — 한쪽을 튜닝해도 다른 경로가 같이 흔들리지 않도록 세 경로를 완전히 분리했다(메시지·통화방은 기본치 범위는 같지만 배율이 달라 서로 별개로 움직인다).

| 경로 | 배율 | 쿨다운 | 비고 |
|---|---|---|---|
| 메인 텍스트 채널 | 배율 없음 (기본치 15~25 그대로, MEE6 기본값) | 30분 | 지정된 채널에서만 인정. 1회 지급량은 원래 그대로 유지하고, 쿨다운을 30분으로 늘려 시간당 최대 2회 × 평균 20~40 XP ≈ 40~80 XP로 억제(통화방 체류의 약 1.67배) — 예전(60초 쿨다운) 시간당 최대 ~1,200 XP였던 것 대비 약 15~30배 낮춤 |
| TTS 채널(통화 연동 전용 채팅방) | 배율 없음 (기본치 15~25 그대로) | 30분 | 메인 채널과 배율·쿨다운 완전히 통합. 이미 음성 XP를 받는 유저는 중복 지급 안 함 |
| 통화방(음성) 체류 | 0.02배 | 1분마다 자동 지급 | 기본치(15~25)는 메시지와 동일, 배율만 낮춰 방치형 XP답게 차등(시간당 평균 24~48 XP). 음소거/헤드셋오프 시 제외, 소수점은 다음 틱으로 이월해 손실 없이 누적 |
| 내전/모집 완료 보너스 | 주최자 1.3배 (참가자는 배율 없음) | 1회성 | 전용 기본치 30~50 XP 사용 — 참가자 평균 40, 주최자 평균 52(범위 39~65). 지정된 채널의 매치가 마감될 때 지급, 유저당 중복 지급 없음 |
...
- **`/xp`**(관리자 전용, `ADMIN_IDS`) — 커맨드에 옵션이 없고 실행하면 본인에게만(ephemeral) **`⚙️ XP 조정` / `🛠️ XP 관리`** 두 버튼이 뜸:
  - **`⚙️ XP 조정`** → **유저 선택 메뉴**. 유저를 고르면 그 유저의 레벨 현황(레벨·진행바·누적 XP)과 함께 `➕`(XP 추가) · `➖`(XP 차감) · `🔄 레벨 조정` · `🚫 XP 지급 정지` 버튼이 달린 패널로 바뀜:
    - `➕`/`➖` → 숫자 입력 모달 → `adjustXp`로 가감
    - `🔄 레벨 조정` → 목표 레벨 입력 모달 → `setXp`로 그 레벨의 시작 XP(`xpForLevelStart`)로 맞춤
    - `🚫 XP 지급 정지`(→ 켜져 있으면 `✅ XP 지급 재개`) — 이 유저 한 명만 콕 집어 자동 XP **지급**(양수)만 막음(`setUserXpFrozen`, 확인 화면 없이 즉시 토글). `applyXp()` 한 곳에서 걸러내므로 메시지·통화방 체류·내전/모집 완료 보너스·미니게임·퀴즈·룰렛 등 **자동 지급 경로 전부**에 적용되고, 정지 중에도 관리자가 `➕`/`➖`/`🔄`로 수동 조정하는 건 그대로 가능(`adjustXp`/`setXp`는 `applyXp`를 거치지 않음). 음수(차감)는 막지 않으므로 오목/틱택토/끝말잇기 내기에서 정지된 유저가 져도 XP는 정상적으로 잃음(제로섬 유지). 상태는 `DB/xp-state.json`에 길드·유저별로 저장돼 재시작해도 유지되고, 패널 상단에 정지 중임을 표시
  ...
  - **`🛠️ XP 관리`** → XP 시스템 스위치 패널(상태는 `DB/xp-state.json`에 저장돼 재시작해도 유지):
    ...
    - **뉴비부스트 ON/OFF** — `NEWBIE_BOOST_ROLE_ID` 역할의 2배 배율을 즉시 켜고 끔(확인 없음)
...
| `applyXp(guildId, userId, amount)` | XP를 더하고 레벨업 여부 반환 (내부 공통 로직). `amount`가 양수이고 `isUserXpFrozen()`이면 아무것도 하지 않고 반환 — 자동 지급 경로 전부가 이 함수를 거치므로 유저별 XP 지급 정지가 여기 한 곳에서 걸림. 음수(차감)는 정지 중에도 그대로 적용됨(오목/틱택토/끝말잇기 내기 정산이 패자에게 `-금액`, 승자에게 `+금액`을 따로 호출하는데, 부호 구분 없이 막으면 정지된 패자가 내기에 져도 XP를 안 잃어 제로섬이 깨짐) |
...
| `loadXpState()` | 봇 시작 시 `DB/xp-state.json`에서 긴급정지·뉴비부스트·유저별 정지 목록 상태 복원(없거나 깨졌으면 전부 정상 작동으로 시작) |
...
| `isUserXpFrozen(guildId, userId)` / `setUserXpFrozen(guildId, userId, frozen)` | `/xp` → `XP 조정`용 — 유저 한 명만 자동 XP 지급을 막는 토글(즉시 디스크 저장). `applyXp()` 안에서 한 번만 확인하므로 자동 지급 경로 전부에 적용되고, 관리자의 수동 조정(`adjustXp`/`setXp`)은 영향받지 않음 |
...
- `/끝말잇기`, `/틱택토`, `/오목`, `/룰렛`, `/레벨`, `/랭킹`과 관련 버튼은 `ALLOWED_CHANNEL_IDS`와 무관하게 `config.js`의 `WORDCHAIN_RANKING_CHANNEL_ID`(= 놀이터) 채널에서만 사용할 수 있습니다.
