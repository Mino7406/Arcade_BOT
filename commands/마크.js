...
function buildRealmPanelPayload() {
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: '승인 시 화이트리스트 역할이 자동으로 지급돼요.' });

  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('realm:apply').setLabel('🏰 신청하기').setStyle(ButtonStyle.Success),
    new ButtonBuilder().setCustomId('realm:roster').setLabel('📋 명단').setStyle(ButtonStyle.Secondary),
  );
    ...
    .addComponents(
      new ActionRowBuilder().addComponents(
        new TextInputBuilder()
          ...
          .setLabel('마인크래프트 닉네임 (Java / Bedrock)')
...
function buildRealmReviewPayload(applicant, nickname, appliedAt, status = 'pending', processedBy = null) {
  const statusMeta = {
    pending:  { title: '🕒 검토 대기중', color: PENDING_COLOR },
    approved: { title: '✅ 승인됨',      color: REALM_COLOR },
    rejected: { title: '❌ 거절됨',      color: REJECTED_COLOR },
  }[status];

  const ts = Math.floor(appliedAt / 1000);
  let footerText = `신청자 ID: ${applicant.id}`;
  if (processedBy) footerText += ` · 처리자: ${processedBy.tag}`;

  const embed = new EmbedBuilder()
    .setColor(statusMeta.color)
    .setAuthor({ name: `${applicant.tag} 님의 렐름 신청서`, iconURL: applicant.displayAvatarURL() })
    .setTitle(statusMeta.title)
    .addFields(
      { name: '🧱 마인크래프트 닉네임', value: nickname, inline: true },
      { name: '📅 신청 일시', value: `<t:${ts}:f>`, inline: true },
    )
  ...
  const components = status === 'pending'
    ? [
        new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId(`realm:approve:${applicant.id}`).setLabel('✅ 승인').setStyle(ButtonStyle.Success),
          new ButtonBuilder().setCustomId(`realm:reject:${applicant.id}`).setLabel('❌ 거절').setStyle(ButtonStyle.Danger),
        ),
      ]
    : [];
...
// 한글/영문이 섞여도 열이 맞도록, 한글(및 기타 전각 문자)은 폭 2로 센다(고정폭 폰트 기준).
...
// 순번 | 디스코드 이름 | 닉네임 표를 코드블록(고정폭 글꼴)으로 그린다.
function buildRosterTable(entries) {
  const headers = ['순번', '디스코드 이름', '닉네임'];
  const rows = entries.map((e, i) => [String(i + 1), e.discordName ?? '알 수 없음', e.nickname]);

  const colWidths = headers.map((h, ci) =>
    Math.max(visualWidth(h), ...rows.map(r => visualWidth(r[ci]))),
  );

  const headerLine = headers.map((h, ci) => padVisual(h, colWidths[ci])).join(' | ');
  const sepLine = [...headerLine].map(ch => (ch === '|' ? '|' : '-')).join('');
  const rowLines = rows.map(r => r.map((cell, ci) => padVisual(cell, colWidths[ci])).join(' | '));
...
function buildRealmRosterMessagePayload(entries) {
  if (entries.length === 0) {
    const embed = new EmbedBuilder().setColor(REALM_COLOR).setTitle('📋 렐름 승인 명단 (0명)')
  ...
  const embed = new EmbedBuilder()
    .setColor(REALM_COLOR)
    .setTitle(`📋 렐름 승인 명단 (총 ${entries.length}명)`)
    .setDescription(
      entries.length > ROSTER_DISPLAY_LIMIT
        ? `${buildRosterTable(display)}\n_...외 ${entries.length - ROSTER_DISPLAY_LIMIT}명 더 있음_`
        : buildRosterTable(display),
    );
...
function buildRealmResultDm(status, nickname) {
  const approved = status === 'approved';
  const container = new ContainerBuilder()
    .addTextDisplayComponents(td => td.setContent(approved ? '# ✅ 렐름 신청 승인' : '# ❌ 렐름 신청 거절'))
    .addSeparatorComponents(new SeparatorBuilder().setDivider(true).setSpacing(SeparatorSpacingSize.Small))
    .addTextDisplayComponents(td => td.setContent(
      approved
        ? `**마인크래프트 렐름 참가 신청**이 승인되었습니다!\n\n🧱 닉네임: \`${nickname}\``
        : `**마인크래프트 렐름 참가 신청**이 아쉽게도 거절되었습니다.\n\n🧱 닉네임: \`${nickname}\``,
    ))
    .addSeparatorComponents(new SeparatorBuilder().setDivider(true).setSpacing(SeparatorSpacingSize.Small))
    .addTextDisplayComponents(td => td.setContent(
      approved
        ? '-# 📬 관리자가 승인 처리하여 자동으로 전송되었습니다. 접속 정보는 곧 안내드릴게요.'
        : '-# 📬 관리자가 거절 처리하여 자동으로 전송되었습니다.',
    ));
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    await interaction.reply({ content: '✅ **렐름 신청 패널을 게시했습니다.**', flags: MessageFlags.Ephemeral });
  },
...
// ── "🏰 신청하기" 버튼 → 신청서 모달 / "✅ 승인"·"❌ 거절" 처리 ('realm:' 프리픽스) ──
async function handleRealmButton(interaction) {
  ...
  if (interaction.customId === 'realm:roster') {
    if (!ADMIN_IDS.includes(interaction.user.id)) {
      await interaction.reply({ content: '❌ **권한이 없습니다.**', flags: MessageFlags.Ephemeral });
      return;
    }
    if (!REALM_REVIEW_CHANNEL_ID) {
      await interaction.reply({ content: '⚠️ **검토 채널이 아직 설정되지 않았습니다.**', flags: MessageFlags.Ephemeral });
      return;
    }
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    await refreshRealmRosterMessage(interaction.client, interaction.guildId);
    await interaction.editReply(`✅ **검토 채널의 명단을 최신 상태로 갱신했습니다.** (<#${REALM_REVIEW_CHANNEL_ID}>)`);
    return;
  }
  ...
  if (!applicant) {
    await interaction.followUp({ content: '⚠️ **신청자 정보를 찾을 수 없습니다(탈퇴 등).**', flags: MessageFlags.Ephemeral });
    return;
  }

  // 원본 임베드에서 닉네임/신청 일시를 그대로 읽어와 재사용한다(별도 저장소 없이 메시지 자체가 상태를 들고 있음).
  const original = interaction.message.embeds[0];
  const nickname = original.fields.find(f => f.name === '🧱 마인크래프트 닉네임')?.value ?? '알 수 없음';
  const appliedAtField = original.fields.find(f => f.name === '📅 신청 일시')?.value ?? '';
  ...
  if (action === 'approve') {
    if (REALM_WHITELIST_ROLE_ID) {
      ...
      if (member) await member.roles.add(REALM_WHITELIST_ROLE_ID).catch(err => console.error('렐름 화이트리스트 역할 지급 실패:', err));
    }
    addApprovedMember(interaction.guildId, applicantId, {
      nickname,
      discordName: applicant.tag,
      approvedAt: Date.now(),
      approvedById: interaction.user.id,
    });
    await applicant.send(buildRealmResultDm('approved', nickname)).catch(err => logRealmDmFailure(err, applicant));
  } else {
    removeApprovedMember(interaction.guildId, applicantId); // 승인 후 재처리로 거절된 경우 명단에서 제거
    await applicant.send(buildRealmResultDm('rejected', nickname)).catch(err => logRealmDmFailure(err, applicant));
  }

  await interaction.editReply(
    buildRealmReviewPayload(applicant, nickname, appliedAt, action === 'approve' ? 'approved' : 'rejected', interaction.user),
  );
...
async function handleRealmModal(interaction) {
  ...
  await reviewChannel.send(buildRealmReviewPayload(interaction.user, nickname, appliedAt, 'pending'));

  await interaction.reply({
    content: `✅ **신청이 접수되었습니다.** (닉네임: \`${nickname}\`)\n검토 후 결과를 DM으로 안내드릴게요.`,
