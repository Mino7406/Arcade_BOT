...
module.exports = {
  ...
  // [임시] /명단관리테스트(테스트 서버 전용)의 "신청서 패널 새로고침" 버튼이 갱신할, 본서버 모집글에
  // 이미 게시된 렐름 신청서 패널 메시지. APPLY_DISABLED를 바꾼 뒤 이 메시지를 그 자리에서 다시
  // 그리는 용도. 채우지 않으면(['']) 새로고침이 "패널 메시지 설정 안 됨" 안내만 낸다.
